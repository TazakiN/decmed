...
use crate::{
    types::{
        AdministrativeData, AppState, HospitalPersonnelRole, KeyNonce, MoveAdministrative,
        PrivateAdministrativeData, PrivateAdministrativeMetadata, PublicAdministrativeData,
        ResponseStatus, SuccessResponse,
    },
    utils::{
        aes_decrypt, aes_encrypt_custom_key, compute_pre_keys, construct_pt,
        construct_shared_object_call_arg, generate_iota_keys_ed, handle_error_move_call_read_only,
        move_call_read_only, parse_keys_entry, parse_move_read_only_result, sha_hash,
    },
};
...
pub async fn is_signed_in(
    ...
    if keys_entry.iota_key_pair.is_none() || keys_entry.pre_secret_key.is_none() {
        return Err("No expected key found".to_string());
    }
    ...
    let pt = construct_pt(
        "get_administrative_data_hospital_personnel".to_string(),
    ...
    let hospital_name: Option<String> = parse_move_read_only_result(response.clone(), 2)?;
    let role: String = parse_move_read_only_result(response.clone(), 1)?;

    let mut role_type = HospitalPersonnelRole::Admin;
    if role == "MedicalPersonnel".to_string() {
        role_type = HospitalPersonnelRole::MedicalPersonnel
    } else if role == "AdministrativePersonnel".to_string() {
        role_type = HospitalPersonnelRole::AdministrativePersonnel
    } else if role != "Admin".to_string() {
        return Err("Invalid role".to_string());
    };

    state.auth_state.role = Some(role_type);
    ...
    public_administrative_data.hospital_name = hospital_name;
...
pub async fn signin(
    ...
    let mnemonic = match Mnemonic::from_str(seed_words.as_str()) {
        Ok(m) => m,
        Err(_) => return Err("Invalid mnemonic".to_string()),
    };
    let seed = mnemonic.to_seed_normalized(keys_entry.id.as_ref().unwrap().as_str());
