...
use crate::types::{
    AccessKeys, AppState, AuthRole, CurrentUser, GenerateSignatureHandlerPayload,
    GetNonceHandlerPayload, HandlerReencryptPayload, JwtClaims, MedicalMetadata,
    MoveHospitalPersonnelRole, ReencryptionPurposeType,
};
...
impl Handlers {
    ...
    pub async fn reencrypt(
        ...
        Json(payload): Json<HandlerReencryptPayload>,
    ) -> Result<Response, ProxyError> {
        ...
        let (enc_medical_data, access_keys, c_frag, prev_index, next_index) = {
            let mut conn = state.redis_pool.get().context(current_fn!())?;

            let access_keys: String = conn
                .get(format!(
                    "keys:{}@{}",
                    current_user.iota_address, payload.patient_iota_address,
                ))
            ...
            let patient_pre_public_key: PublicKey =
                Utils::serde_deserialize_from_base64(access_keys.patient_pre_public_key.clone())
                    .context(current_fn!())?;
            let hospital_personnel_pre_public_key: PublicKey =
                Utils::serde_deserialize_from_base64(
                    access_keys.hospital_personnel_pre_public_key.clone(),
                )
                .context(current_fn!())?;
            let medical_metadata_key_nonce_capsule: Capsule =
                Utils::serde_deserialize_from_base64(medical_metadata.capsule)
                    .context(current_fn!())?;
            ...
            let verified_kfrag = k_frag
                .verify(
                    ...
                    Some(&hospital_personnel_pre_public_key),
                )
        ...
        let res_data = json!({
            "c_frag": Utils::serde_serialize_to_base64(&c_frag),
            "enc_hospital_personnel_pre_secret_key_seed": access_keys.enc_hospital_personnel_pre_secret_key_seed,
            "enc_medical_data": enc_medical_data,
            "hospital_personnel_pre_public_key": access_keys.hospital_personnel_pre_public_key,
            "hospital_personnel_pre_secret_key_seed_capsule": access_keys.hospital_personnel_pre_secret_key_seed_capsule,
    ...
    pub async fn store_keys(
        ...
        let access_keys = AccessKeys {
            enc_hospital_personnel_pre_secret_key_seed: payload
                .enc_hospital_personnel_pre_secret_key_seed,
            hospital_personnel_pre_public_key: payload.hospital_personnel_pre_public_key,
            hospital_personnel_pre_secret_key_seed_capsule: payload
                .hospital_personnel_pre_secret_key_seed_capsule,
