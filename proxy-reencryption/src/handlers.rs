...
use crate::types::{
    ...
    HandlerCreateMedicalRecordSegmentPayload,
    HandlerGetAdministrativeDataQueryParams,
    HandlerGetMedicalRecordQueryParams,
    HandlerGetMedicalRecordUpdateQueryParams,
    HandlerListMedicalRecordsQueryParams,
    HandlerStoreKeysPayload,
    HandlerUpdateMedicalRecordPayload,
    MedicalMetadata,
    MedicalRecordMetadataItem,
    MoveHospitalPersonnelRole,
    PatientPrivateAdministrativeMetadata,
    PatientRevocationSignedPayload,
...
fn deserialize_stored_medical_metadata(
    ...
    let segment_metadata: RmeSegmentMetadata = serde_json
        ::from_value(metadata_value)
...
fn revocation_ttl(expires_before: Option<&str>) -> Result<u64, ProxyError> {
    ...
    let expiry = if let Ok(epoch_secs) = expires_before.parse::<i64>() {
        ...
        chrono::DateTime
            ::parse_from_rfc3339(expires_before)
    ...
    let remaining = expiry.signed_duration_since(chrono::Utc::now()).num_seconds();
    if remaining <= 0 {
...
fn get_access_keys_for_current_user(
    ...
    for subject in access_key_subject_candidates(current_user) {
        let stored_access_keys: redis::RedisResult<String> = conn.get(
            format!("keys:{}@{}", subject, patient_iota_address)
        );

        if let Ok(stored_access_keys) = stored_access_keys {
            let access_keys = Utils::serde_deserialize_from_base64(stored_access_keys).context(
                current_fn!()
            )?;
...
impl Handlers {
    pub async fn create_medical_record(
        ...
        let (
            ...
            let patient_iota_address = IotaAddress::from_str(&payload.patient_iota_address)
                ...
                .code(StatusCode::BAD_REQUEST)?;
            let medical_metadata: ClientMedicalMetadata = Utils::serde_deserialize_from_base64(
                payload.medical_metadata
            )
                .map_err(|_| anyhow!("Invalid medical metadata"))
                .code(StatusCode::BAD_REQUEST)?;
            let hospital_personnel_iota_address = IotaAddress::from_str(
                &current_user.iota_address
            ).map_err(|_| anyhow!("Invalid hospital personnel IOTA address"))?;
            let proxy_iota_address = IotaAddress::from_str(&state.proxy_iota_address).context(
                current_fn!()
            )?;
        ...
        let cid = Utils::add_and_pin_to_ipfs(medical_metadata.enc_data).await.context(
            current_fn!()
        )?;
    ...
    pub async fn create_medical_record_segment(
        ...
        let (
            ...
            let patient_iota_address = IotaAddress::from_str(&payload.patient_iota_address)
                .map_err(|_| anyhow!("Invalid patient IOTA address"))
                .code(StatusCode::BAD_REQUEST)?;
            let encrypted_segment: ClientEncryptedRmeSegment = Utils::serde_deserialize_from_base64(
                payload.encrypted_segment
            )
        ...
        let created_at = Utils::sys_time_to_iso(std::time::SystemTime::now());
        let updated_at = encrypted_segment.correction_of_index.map(
            |_| chrono::Utc::now().timestamp_millis().max(0) as u64
        );
        ...
        if current_user.decmed_token.is_some() {
            let wallet_sig = headers.get(WALLET_SIGNATURE_HEADER).and_then(|v| v.to_str().ok());
            ...
            let verifier: Option<&dyn decmed_macaroon_auth::WalletSignatureVerifier> = Some(
                &IotaWalletVerifier
            );
            verify_decmed_token(
                &mac,
        ...
        let cid = Utils::add_and_pin_to_ipfs(encrypted_segment.enc_data.clone()).await.context(
            current_fn!()
        )?;
     ...
     */
    pub async fn generate_and_register_proxy_address(State(state): State<Arc<AppState>>) -> Result<
        Response,
        ProxyError
    > {
        ...
        let (proxy_iota_address, proxy_iota_keypair) = Utils::generate_iota_keys_ed(&seed).context(
            current_fn!()
        )?;
        ...
        let _ = state.move_call
            .create_capability(
                ...
                IotaKeyPair::decode(&state.global_admin_iota_key_pair.clone()).map_err(|e|
     ...
     * This is just helper function
     */
    pub async fn generate_signature(Json(payload): Json<GenerateSignatureHandlerPayload>) -> Result<
        Response,
        ProxyError
    > {
        ...
        Ok(Utils::build_success_response(signature_string, StatusCode::OK))
    }
    ...
    pub async fn get_administrative_data(
        ...
        if
            current_user.role != AuthRole::AdministrativePersonnel &&
        ...
        if
            current_user.purpose != ReencryptionPurposeType::Read &&
            !(
                current_user.role == AuthRole::MedicalPersonnel &&
        ...
        let (hospital_personnel_iota_address, patient_iota_address, proxy_iota_address) = {
            ...
            (hospital_personnel_iota_address, patient_iota_address, proxy_iota_address)
        };
        ...
        let (
            ...
            let patient_administrative_metadata = state.move_call
            ...
            let k_frag: KeyFrag = Utils::serde_deserialize_from_base64(
                access_keys.k_frag.clone()
            ).context(current_fn!())?;
            let signer_pre_public_key: PublicKey = Utils::serde_deserialize_from_base64(
                access_keys.signer_pre_public_key.clone()
            ).context(current_fn!())?;
            let patient_pre_public_key: PublicKey = Utils::serde_deserialize_from_base64(
                access_keys.patient_pre_public_key.clone()
            ).context(current_fn!())?;
            let data_pre_public_key: PublicKey = Utils::serde_deserialize_from_base64(
                access_keys.data_pre_public_key.clone()
            ).context(current_fn!())?;
    ...
    pub async fn get_medical_record(
        ...
        let (
            ...
            let (medical_metadata, administrative_metadata, current_index, prev_index, next_index) =
                loop {
                    ...
                    if current_user.decmed_token.is_some() {
                        if
                            let Ok(segment_meta) =
                                Utils::serde_deserialize_from_base64::<RmeSegmentMetadata>(
                                ...
                                )
                        {
                            ...
                            let wallet_timestamp = headers
                                .get(WALLET_TIMESTAMP_HEADER)
                                .and_then(|v| v.to_str().ok());
                            let mac = Macaroon::deserialize(&current_user.bearer_token).map_err(
                                |_| {
                                    ProxyError::Anyhow {
                                        source: anyhow!("Invalid access token"),
                                        code: StatusCode::UNAUTHORIZED,
            ...
            let enc_medical_data = Utils::get_data_ipfs(medical_metadata.cid).await.context(
                current_fn!()
            )?;
            ...
            let medical_metadata_key_nonce_capsule: Capsule = Utils::serde_deserialize_from_base64(
                medical_metadata.capsule.clone()
            ).context(current_fn!())?;
            ...
            let verified_kfrag = k_frag
                ...
                .map_err(|e| anyhow!(e.0.to_string()).context(current_fn!()))?;
            let verified_cfrag_medical = reencrypt(
                &medical_metadata_key_nonce_capsule,
                verified_kfrag.clone()
            );
            ...
            let administrative = if include_administrative {
                let patient_private_adm_metadata: PatientPrivateAdministrativeMetadata =
                    Utils::serde_deserialize_from_base64(
                        administrative_metadata.private_metadata
                    ).context(current_fn!())?;
                let patient_private_adm_metadata_key_nonce_capsule: Capsule =
                    Utils::serde_deserialize_from_base64(
                        patient_private_adm_metadata.capsule.clone()
            ...
            let (
                ...
                .map(|(enc_data, c_frag, enc_key_nonce, capsule)| {
                    (Some(enc_data), Some(c_frag), Some(enc_key_nonce), Some(capsule))
                })
        ...
        if include_administrative {
            let administrative_data_capsule = administrative_data_capsule.ok_or_else(||
                anyhow!("Missing administrative data")
            )?;
            let c_frag_administrative = c_frag_administrative.ok_or_else(||
                anyhow!("Missing administrative cfrag")
            )?;
            let enc_administrative_data = enc_administrative_data.ok_or_else(||
                anyhow!("Missing administrative payload")
            )?;
            let enc_administrative_data_key_nonce = enc_administrative_data_key_nonce.ok_or_else(||
                anyhow!("Missing administrative key nonce")
            )?;
            ...
            if let Value::Object(map) = &mut res_data {
                ...
                map.insert(
                    ...
                    json!(
                        Utils::serde_serialize_to_base64(&c_frag_administrative).context(
                            current_fn!()
                        )?
                    )
                );
                map.insert("enc_administrative_data".to_string(), json!(enc_administrative_data));
                map.insert(
    ...
    pub async fn list_medical_records(
        ...
        loop {
            ...
            for (offset, record) in page.into_iter().enumerate() {
                let Some(segment) = crate::metadata_list::decode_rme_segment_metadata(
                    &record.metadata
                ) else {
                    continue;
                };
                ...
                if include {
                    filtered.push(
                        crate::metadata_list::to_metadata_item(
                            record.index,
    ...
    pub async fn get_medical_record_update(
        ...
        let (
            ...
            let (medical_metadata, administrative_metadata) = state.move_call
    ...
    pub async fn get_nonce_handler(
        ...
        let _ = state.move_call
            .is_patient_registered(&patient_iota_address, proxy_iota_address).await
            .context(current_fn!())?;
    ...
    pub async fn store_keys(
        ...
        let patient_iota_address = IotaAddress::from_str(&payload.patient_iota_address)
            ...
            .code(StatusCode::BAD_REQUEST)?;
        let hospital_personnel_iota_address = IotaAddress::from_str(
            &payload.hospital_personnel_iota_address
        )
            .map_err(|_| anyhow!("Invalid hospital personnel IOTA address"))
            .code(StatusCode::BAD_REQUEST)?;
        ...
        let _ = signature
            .verify_secure(&intent_message, patient_iota_address, SignatureScheme::ED25519)
        ...
        let role = state.move_call
            .get_hospital_personnel_role(&hospital_personnel_iota_address, proxy_iota_address).await
            .context(current_fn!())?;
        ...
        let (hospital_personnel_role, read_keys_duration, update_keys_duration): (
            ...
            MoveHospitalPersonnelRole::MedicalPersonnel =>
                (AuthRole::MedicalPersonnel, MEDICAL_KEYS_READ_DUR, Some(MEDICAL_KEYS_UPDATE_DUR)),
            _ => {
                return Err(ProxyError::Anyhow {
        ...
        let (hospital_personnel_access_token_read, hospital_personnel_access_token_update) = match
            hospital_personnel_role
        {
            AuthRole::AdministrativePersonnel => {
                ...
                if let Some(encounter_dataset) = payload.encounter_dataset {
                    ...
                    let read_token = issue_admin_personnel_token(&root_key, &read_params).map_err(
                        |e| ProxyError::Caveat {
                            code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                            error: e.to_string(),
                        }
                    )?;
                    let update_token = issue_admin_personnel_token(
                        &root_key,
                        &write_params
                    ).map_err(|e| ProxyError::Caveat {
                        code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
            ...
            AuthRole::MedicalPersonnel => {
                ...
                if let Some(related_rme_id) = payload.related_rme_id.clone() {
                    response_related_rme_id = Some(related_rme_id.clone());
                    let expires_before =
                        chrono::Utc::now() + chrono::Duration::seconds(read_keys_duration as i64);
                    ...
                    let read_token = issue_initial_token(&root_key, &read_params).map_err(|e| {
                        ProxyError::Caveat {
                            code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                            error: e.to_string(),
                        }
                    ...
                    let update_token = if let Some(update_keys_duration) = update_keys_duration {
                        ...
                        let mut update_params = InitialDoctorTokenParams::example_rm_initial_token(
                            &patient_iota_address.to_string(),
                            &related_rme_id,
                            &root_subject
                        ).into_update_only();
        ...
        let access_token_read_hash = decmed_macaroon_auth::hash_token(
            &hospital_personnel_access_token_read
        );
    ...
    pub async fn revoke_patient_access(
        ...
        let patient_iota_address = IotaAddress::from_str(&payload.patient_address)
            .map_err(|_| anyhow!("Invalid patient IOTA address"))
            .code(StatusCode::BAD_REQUEST)?;
        let _proxy_iota_address = IotaAddress::from_str(&state.proxy_iota_address).context(
            current_fn!()
        )?;
        ...
        let signature = Utils::construct_signature_from_str(&payload.signature)
            .map_err(|_| anyhow!("Invalid signature"))
            .code(StatusCode::BAD_REQUEST)?;
        let canonical = serde_json
            ::to_string(&PatientRevocationSignedPayload::from(&payload))
            .map_err(|e| anyhow!(e.to_string()))
            .code(StatusCode::BAD_REQUEST)?;
        ...
        let _: usize = conn
            .del(format!("keys:{}@{}", payload.root_subject, payload.patient_address))
            .context(current_fn!())?;
