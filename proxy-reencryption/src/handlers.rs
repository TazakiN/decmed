...
impl Handlers {
    pub async fn reserve_related_rme_id_handler(State(state): State<Arc<AppState>>) -> Result<
        Response,
        ProxyError
    > {
            ...
            hospital_personnel_role
        {
            AuthRole::AdministrativePersonnel => {
                ...
                if let Some(encounter_dataset) = payload.encounter_dataset {
                    ...
                    let expires_before = requested_expiry
                        .clone()
                        .unwrap_or_else(|| {
    ...
    pub async fn revoke_patient_access(
        ...
        let token_hash = payload.token_hash.as_ref().ok_or_else(|| ProxyError::Anyhow {
