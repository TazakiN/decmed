...
fn sub_role_can_write_dataset(
    ...
    match sub_role {
        HospitalPersonnelSubRole::Doctor | HospitalPersonnelSubRole::Nurse =>
            matches!(dataset_category, DatasetCategory::RAWAT_JALAN | DatasetCategory::RAWAT_INAP),
...
pub fn authorize_segment_hospital(
    ...
    let token_hospital_cid = token_hospital_cid.ok_or_else(|| {
        map_caveat_error(CaveatVerificationError::MissingRequiredCaveat("hospital_cid"))
    })?;

    if token_hospital_cid != segment_hospital_cid {
        return Err(map_caveat_error(CaveatVerificationError::HospitalCidMismatch));
    }
...
mod tests {
    ...
    fn doctor_and_nurse_cannot_write_laboratorium_or_apotek() {
        for sub_role in [HospitalPersonnelSubRole::Doctor, HospitalPersonnelSubRole::Nurse] {
    ...
    fn assert_proxy_status(err: ProxyError, expected: StatusCode) {
        match err {
            ...
            ProxyError::Caveat { code, .. } => { assert_eq!(code, expected.as_u16()) }
